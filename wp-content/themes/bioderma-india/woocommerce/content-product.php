...
if($product->get_attribute( 'Price' ) !=""){
  $priceStr = $product->get_attribute( 'price' );
...
$category='';
if($terms[0]->name!=""){$category = $terms[0]->name;}
?>
   ...
   <?php do_action('woocommerce_before_shop_loop_item'); ?>
    ...
    <a href="<?php the_permalink(); ?>">
          <div class="labels labels--absolute">
            ...
            <figure class="lazyload-wrapper">
		        <img class="lazyload attachment-post-thumbnail size-post-thumbnail wp-post-image products-list__thumb" data-src="<?php echo   the_post_thumbnail_url( $size );?>" src="<?php echo   the_post_thumbnail_url( $size );?>" alt="<?php echo  get_the_title();?>" data-srcset="<?php echo   the_post_thumbnail_url( $size );?> 300w, <?php echo   the_post_thumbnail_url( $size );?> 1024w" data-sizes="(max-width:1500px) 100vw, 1500px">
		        <div class="aspect-ratio" style="padding-top: 120%"></div>
		       </figure>
		    <h1 class="products-list__title accent-medium">
		    <?php echo $category;?>
		    <br />
            <?php echo preg_replace( '/\w+/', '', get_the_title(), 2 );?>
		    </h1>
		    <span class="products-list__starting primary-medium">Starting at &#x20B9;<?php echo $priceArr[0]; 
		    ?>
		    </span>
        </a>
      </div>
    ...
    <div class="products-list__grid grid grid--column grid--spaced">
      <div class="g-item">
        <p class="products-list__title accent-medium">
        <?php echo $category;?>
        <br />
        <?php echo preg_replace( '/\w+/', '', get_the_title(), 2 );?>
        </p>
      ...
      <div class="g-item">
                  <span class="products-list__starting primary-medium">Starting at &#x20B9;<?php echo  $priceArr[0]; //do_action('woocommerce_after_shop_loop_item_title');
                   ?></span>
