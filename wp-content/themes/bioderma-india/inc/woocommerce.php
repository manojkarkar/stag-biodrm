...
/*TQ-themes-functions*/
function tq_woocommerce_single_product_gallery() {
	global $post, $product;
	$attachments_ids = array();
	$attachments_ids =  $product->get_gallery_attachment_ids();
	if(empty($attachments_ids)) return ;
	echo '<div class="g-item product-gallery__thumbs grid grid--column">';
	foreach($attachments_ids as $attachments_id) {
		    $full_image_url = wp_get_attachment_url($attachments_id);
		?>

   			<a class="product-gallery__thumb" href="<?php echo $full_image_url;?>">
            <figure class="lazyload-wrapper">
             <img class="lazyload attachment-post-thumbnail size-post-thumbnail wp-post-image products-list__thumb" data-src="<?php echo $full_image_url;?>" alt="" data-srcset="<?php echo $full_image_url;?> 300w, <?php echo $full_image_url;?> 1000w" data-sizes="(max-width:1000px) 100vw, 1000px">
...
add_action('tq_woocommerce_single_product_left', 'tq_woocommerce_single_product_gallery', 5);
...
add_action('tq_woocommerce_output_related_post', 'woocommerce_output_related_post');
...
function woocommerce_output_related_post(){
		...
		if ( ! empty( $terms ) && ! is_wp_error( $terms ) ){
			    foreach ( $terms as $term ) {
			        $term_array[] = $term->name;
			      	$page = get_posts(array('tag' => $term->name));     
			    }
			}
			if(! empty( $page )):
				$postEl = array_pop((array_slice($page, -1)));
		  		$shortDesc = "";
				if($postEl->post_excerpt != ""){
					$shortDesc = $postEl->post_excerpt;
				}
				$cat_name = $post_image = "";
				$post_image = wp_get_attachment_image_src( get_post_thumbnail_id( $postEl->ID ), 'single-post-thumbnail' ); 
				$cat_name = get_the_category( $postEl->ID )[0]->name;
			?>
			<section>
				    ...
				    <div class="">
				        <article class="">
				          <div class="wrap grid grid--article-slideshow grid--spaced">
				            <div class="g-item">
				              <figure class="lazyload-wrapper">
				                <img class="attachment-post-thumbnail size-post-thumbnail wp-post-image article-slideshow__big-media lazyloaded" data-src="<?php echo $post_image[0]; ?>" alt="" data-srcset="<?php echo $post_image[0]; ?> 300w, <?php echo $post_image[0]; ?> 768w, <?php echo $post_image[0]; ?> 1024w, <?php echo $post_image[0]; ?> 1600w" data-sizes="(max-width: 1600px) 100vw, 1600px" sizes="(max-width: 1600px) 100vw, 1600px" srcset="<?php echo $post_image[0]; ?> 300w, <?php echo $post_image[0]; ?> 768w, <?php echo $post_image[0]; ?> 1024w, <?php echo $post_image[0]; ?> 1600w" src="<?php echo $post_image[0]; ?>">
				            ...
				            <div class="g-item article-slideshow__content grid grid--center grid--column">
				              	<div>
					                ...
					                <h1 class="h1 accent article-slideshow__title"><?php echo $postEl->post_title; ?></h1>
					                <p class="article-slideshow__excerpt"><?php echo $shortDesc; ?></p>
					                <a class="btn btn--outline" href="<?php echo get_page_uri($postEl->ID); ?>">Read more</a>
				                </div>
				            </div>
				          </div>
				        </article>
